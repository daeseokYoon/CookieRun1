...
public class MYCOOKIES_Main : MonoBehaviour //code by. 하은
{
    ...
    void Start()
    {
        ...
        EventManager.instance.onSelectBtnClick = (id) => {
            ...
            //클릭한 오브젝트의 데이터가 있는 object의 cheakObject를 true 상태로 만들어줌.
            UserDataManager.Instance.SetSelectTreasureID(id); // -----------------
            ...
            //그리고 체크아이콘이 있는 프리팹의 id를 가져옴. // 리스트에 추가된 프리팹의 정보가 있다면 거기서 GetID함수에 버튼으로 받아온 int값을 넣어줌
            ...
            //쿠키를 가지고 있고 불러온 id정보가 같으면 체크 아이콘은 계속 true,
            // 다른 UIScrollViewCookiesSelect가 있는 프리팹을 선택했다면 다른 프리팹에 켜져있는
            // 체크표시가 false
